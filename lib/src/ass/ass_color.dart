...
List<int> assColorToRGB(String hexColor) {
  if (hexColor.toUpperCase().trim().startsWith('&H') &&
      hexColor.toUpperCase().trim().endsWith('&')) {
  ...
  try {
    ...
    throw FormatException(
      "Invalid entry. Make sure all characters are hexadecimal.",
    );
  }
}
...
class AssColor {
  ...
  factory AssColor.parseLibassStyleColor(String assColor) {
    var hexColor = assColor.toUpperCase().trim();
    if (hexColor.startsWith('&H')) {
      hexColor = hexColor.substring(2);
    }
    if (hexColor.endsWith('&')) {
    ...
    try {
      return AssColor(
        red: int.parse(hexColor.substring(4, 6), radix: 16),
        green: int.parse(hexColor.substring(2, 4), radix: 16),
        blue: int.parse(hexColor.substring(0, 2), radix: 16),
      );
    } catch (e) {
      throw FormatException(
        "Invalid entry. Make sure all characters are hexadecimal.",
  ...
  String toLibassStyleColor() {
    ...
    if (red != null && green != null && blue != null) {
      ...
      bff.write('$blueHex$greenHex$redHex'.toUpperCase());
    }
    return bff.toString();
  }
